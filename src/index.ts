  ...
  constructor(
    ...
    const _remoteFilesChangeHandler = (events: Y.YMapEvent<any>, txn: Y.Transaction) => {
      ...
      if (middleware?.transformRemoteFiles) {
        ...
        if (res.length !== addedFiles.length) {
          console.warn(
            "ExcalidrawBinding: transformRemoteFiles middleware returned a different number of files than expected. This may lead to unexpected behavior."
          );
        }
    ...
    if (middleware?.transformRemoteFiles) {
      ...
      if (res) {
        if (res.length !== initialAssets.length) {
          // A simple sanity check is enough, no need to compare ids
          console.warn(
            "ExcalidrawBinding: transformRemoteFiles middleware returned a different number of files than expected. This may lead to unexpected behavior."
          );
        }
